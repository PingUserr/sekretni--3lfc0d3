...
public class StringButton
    ...
    public void drawScreen(GuiGraphics context, int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isListening) {
            ...
            String label = this.setting.getName().equals("Buttons") ? "Buttons " : (this.setting.getName().equals("Prefix") ? "Prefix  " : this.setting.getName() + " ");
            drawString(label, ChatFormatting.GRAY + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float) OyVeyGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406, -1);
        }
    }
