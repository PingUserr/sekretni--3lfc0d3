...
public abstract class HudModule extends Module {
    ...
    protected void render(Render2DEvent e) {
        ...
        if (shouldDrawDescription) {
            float textWidth = OyVey.fontManager.getStringWidth(getName());
            int textHeight = OyVey.fontManager.getFontHeight();
            float textX = x + width + 5;
            if (textX + textWidth > mc.getWindow().getGuiScaledWidth()) {
                textX = x - 5 - textWidth;
            }
            OyVey.fontManager.drawString(e.getContext(), getName(),
                    textX, y + height / 2f - textHeight / 2f, -1);
